...
namespace App\MoonShine\Resources\SpacexLdnDcDbMetricReport\Pages;
...
use App\Services\SpacexLdnDcDbMetricReportService;
use Carbon\Carbon;
...
/**
 * @extends FormPage<SpacexLdnDcDbMetricReportResource>
 */
class SpacexLdnDcDbMetricReportFetchPage extends FormPage
{
    use GuardsFetchPageAccess;

    public function getTitle(): string
    {
        return 'Fetch Data DB Metric (DC) dari Elasticsearch (via Grafana)';
    }
    ...
    protected function mainLayer(): array
    {
        ...
        return [
            Heading::make('Fetch Data DB Metric (DC) dari Elasticsearch (via Grafana)'),
            ...
            Alert::make(type: 'info')
                ->content('⏳ Data diambil dari index <strong>metricbeat-*</strong> (agent.name: REPODBLDNDC), metric CPU/Memory/Filesystem.'),
            ...
            Div::make([
                ...
                FormBuilder::make()
                    ...
                    ->name('spacex-ldn-dc-db-metric-fetch-form')
                    ->fields([
                        Flex::make([
                            ...
                            ActionButton::make()
                                ...
                                ->dispatchEvent([
                                    AlpineJs::event(JsEvent::FORM_SUBMIT, 'spacex-ldn-dc-db-metric-fetch-form'),
                                ]),
                        ])->unwrap(),
                    ])
                    ->hideSubmit(),
    ...
    public function fetchManual(): JsonResponse
    {
        ...
        try {
            $service = app(SpacexLdnDcDbMetricReportService::class);
            $success = 0;
            $failed  = 0;
            $current = $from->copy();
