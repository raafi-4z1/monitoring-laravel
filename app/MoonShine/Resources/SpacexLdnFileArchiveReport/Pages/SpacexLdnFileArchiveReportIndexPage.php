...
/**
 * @extends IndexPage<SpacexLdnFileArchiveReportResource>
 */
class SpacexLdnFileArchiveReportIndexPage extends IndexPage
{
    ...
    public function changePerPage(): JsonResponse
    {
        ...
        if ($perPage > 0) {
            session(['spacexLdnFileArchivePerPage' => $perPage]);
        }
    ...
    {
        $latest = SpacexLdnFileArchiveReport::latest('trx_date')->first();
     ...
     * SpacexLdnFileArchiveReport::fileGroup() (bagian stabil sebelum titik pertama), bukan
    ...
    private function buildRowCountChart(): Grid
    {
        ...
        $rows = SpacexLdnFileArchiveReport::query()
        ...
        $groups = $rows->groupBy(fn(SpacexLdnFileArchiveReport $r) => $r->fileGroup())->sortKeys();

        $chart = LineChartMetric::make('Row Count per Tanggal (' . self::CHART_DAYS . ' hari terakhir)')
            ->withoutSortKeys();

        foreach ($groups as $groupName => $groupRows) {
            $byDate = $groupRows->keyBy(fn(SpacexLdnFileArchiveReport $r) => $r->trx_date->format('Y-m-d'));
