...
namespace App\MoonShine\Resources\SpacexNycDcAppMetricReport\Pages;
...
/**
 * @extends FormPage<SpacexNycDcAppMetricReportResource>
 */
class SpacexNycDcAppMetricReportFetchPage extends FormPage
{
    use GuardsFetchPageAccess;

    public function getTitle(): string
    {
        return 'Fetch Data APP Metric (DC) New York dari Elasticsearch (via Grafana)';
    }
    ...
    protected function mainLayer(): array
    {
        ...
        return [
            Heading::make('Fetch Data APP Metric (DC) New York dari Elasticsearch (via Grafana)'),
            ...
            Alert::make(type: 'info')
                ->content('⏳ Data diambil dari index <strong>metricbeat-*</strong> (agent.name: HQREPONYADC), metric CPU/Memory/Filesystem (drive Z: dikecualikan).'),
            ...
            Div::make([
                ...
                FormBuilder::make()
                    ...
                    ->name('spacex-nyc-dc-app-metric-fetch-form')
                    ->fields([
                        Flex::make([
                            ...
                            ActionButton::make()
                                ...
                                ->dispatchEvent([
                                    AlpineJs::event(JsEvent::FORM_SUBMIT, 'spacex-nyc-dc-app-metric-fetch-form'),
                                ]),
                        ])->unwrap(),
                    ])
                    ->hideSubmit(),
    ...
    public function fetchManual(): JsonResponse
    {
        ...
        try {
            $service = app(SpacexNycDcAppMetricReportService::class);
            $success = 0;
            $failed  = 0;
            $current = $from->copy();
