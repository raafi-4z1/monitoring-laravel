...
namespace App\MoonShine\Resources\SpacexNycJobExecutionReport\Pages;
...
use App\Services\SpacexNycJobExecutionReportService;
use Carbon\Carbon;
...
/**
 * @extends FormPage<SpacexNycJobExecutionReportResource>
 */
class SpacexNycJobExecutionReportFetchPage extends FormPage
{
    use GuardsFetchPageAccess;

    public function getTitle(): string
    {
        return 'Fetch Data Job Execution (New York) dari Elasticsearch (via Grafana)';
    }
    ...
    protected function mainLayer(): array
    {
        ...
        return [
            Heading::make('Fetch Data Job Execution (New York) dari Elasticsearch (via Grafana)'),
            ...
            Alert::make(type: 'info')
                ->content('⏳ Data diambil dari index <strong>reportingkcln-*</strong> (log_category: BATCH_JOB, server: newyork_dc), khusus job <strong>Batch_edw.sh</strong> dan <strong>run_edw_dblink.sh</strong>.'),
            ...
            Div::make([
                ...
                FormBuilder::make()
                    ...
                    ->name('spacex-nyc-job-execution-fetch-form')
                    ->fields([
                        Flex::make([
                            ...
                            ActionButton::make()
                                ...
                                ->dispatchEvent([
                                    AlpineJs::event(JsEvent::FORM_SUBMIT, 'spacex-nyc-job-execution-fetch-form'),
                                ]),
                        ])->unwrap(),
                    ])
                    ->hideSubmit(),
    ...
    public function fetchManual(): JsonResponse
    {
        ...
        try {
            $service = app(SpacexNycJobExecutionReportService::class);
            $success = 0;
            $failed  = 0;
            $current = $from->copy();
