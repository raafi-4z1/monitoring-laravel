...
/**
 * @extends IndexPage<WicAppMetricReportResource>
 */
class WicAppMetricReportIndexPage extends IndexPage
{
    ...
    protected function modifyListComponent(ComponentContract $component): ComponentContract
    {
        return $component
            ...
            ->stickyButtons()
            ->async(events: [
                AlpineJs::event(JsEvent::FRAGMENT_UPDATED, 'wic-app-metric-charts'),
            ])
            ->topRight(function () {
                return [
                    Div::make([
    ...
    public function changePerPage(): JsonResponse
    {
        ...
        if ($perPage > 0) {
            session(['wicAppMetricPerPage' => $perPage]);
        }
    ...
    {
        $latest = WicAppMetricReport::latest('trx_date')->latest('trx_hour')->first();
        return $latest
    ...
    private function buildCharts(Collection $data, string $period, ?string $metricTypeFilter): Grid
    {
        ...
        $cpuP95 = $cpuData->mapWithKeys(fn($r) => [$label($r) => round(($r->p95_pct ?? 0) * 100, 2)])->toArray();
        ...
        $memP95 = $memData->mapWithKeys(fn($r) => [$label($r) => round(($r->p95_pct ?? 0) * 100, 2)])->toArray();
