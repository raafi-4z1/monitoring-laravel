 ...
 * datasource sama persis dengan SpacexLdnDcAppMetricReportService (host beda, cluster ES sama -
 * satu index metricbeat-* menampung semua host). Host di sini REPODBLDNDC (RHEL, server DB),
 * beda OS dari HQREPOLDNDC (Windows) - makanya disk_path di sini berupa mount point Linux
 * (/oradata, /oralog, /u01, dst.), bukan drive letter, dan TIDAK ADA pengecualian "Z:" karena
 * itu spesifik konsep Windows network drive yang tidak relevan di host Linux ini.
 */
class SpacexLdnDcDbMetricReportService
{
    public const SERVICE_NAME = 'db_storage_dc';
    ...
    public function fetchAndStore(Carbon $date): bool
    {
        ...
        if ($reportSource === null || (blank($reportSource->host_ip) && blank($reportSource->service_integrator))) {
            Log::channel('daily')->warning(
                "SpacexLdnDcDbMetricReportService: report_source dengan service_name '" . self::SERVICE_NAME . "' "
        ...
        try {
            ...
            foreach ($cpuData as $hourKey => $v) {
                ...
                SpacexLdnDcDbMetricReport::updateOrCreate(
            ...
            if ($count === 0) {
                Log::warning("SpacexLdnDcDbMetricReport: tidak ada data untuk {$dateStr}");

                return false;
            }

            Log::info("SpacexLdnDcDbMetricReport: berhasil simpan {$count} baris untuk {$dateStr}");

            return true;
        } catch (\Throwable $e) {
            Log::error("SpacexLdnDcDbMetricReport: gagal - {$e->getMessage()}");
