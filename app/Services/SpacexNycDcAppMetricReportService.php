 ...
 * (GrafanaElasticsearchService), BUKAN cluster ES langsung. Identik dengan
 * SpacexLdnDcAppMetricReportService, host-nya saja yang beda: agent.name "HQREPONYADC" (host.ip
 * 192.168.33.195, diverifikasi live ke Grafana) - padanan server New York untuk HQREPOLDNDC.
 * Sama seperti London, mapping-nya ECS standar jadi agent.name/host.ip/system.filesystem.mount_point
 * SUDAH keyword-type (tidak perlu ".keyword"), dan metricset.name difilter pakai "match" (bukan
 * "term .keyword").
 */
class SpacexNycDcAppMetricReportService
{
    public const SERVICE_NAME = 'app_storage_dc_nyc';
    ...
    public function fetchAndStore(Carbon $date): bool
    {
        ...
        if ($reportSource === null || (blank($reportSource->host_ip) && blank($reportSource->service_integrator))) {
            Log::channel('daily')->warning(
                "SpacexNycDcAppMetricReportService: report_source dengan service_name '" . self::SERVICE_NAME . "' "
        ...
        try {
            ...
            foreach ($cpuData as $hourKey => $v) {
                ...
                SpacexNycDcAppMetricReport::updateOrCreate(
            ...
            if ($count === 0) {
                Log::warning("SpacexNycDcAppMetricReport: tidak ada data untuk {$dateStr}");

                return false;
            }

            Log::info("SpacexNycDcAppMetricReport: berhasil simpan {$count} baris untuk {$dateStr}");

            return true;
        } catch (\Throwable $e) {
            Log::error("SpacexNycDcAppMetricReport: gagal - {$e->getMessage()}");
    ...
    /**
     * NOT system.filesystem.mount_point : "Z:\\" - sama seperti London, drive Z: adalah network
     * share terpetakan yang tidak relevan dipantau (host New York juga tidak punya drive Z: saat
     * diverifikasi live, tapi filter ini tetap aman/tidak berdampak kalau memang tidak ada).
     */
