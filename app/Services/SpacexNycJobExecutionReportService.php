...
class SpacexNycJobExecutionReportService
{
    public const SERVICE_NAME = 'job_execution_nyc';

    /**
     * Cuma 2 job ini yang diambil dari log_category BATCH_JOB - sama seperti Job Execution
     * server London, job lain dalam kategori yang sama (get_f1_file.sh, get_ext_file.sh, dll.)
     * diabaikan.
     */
    ...
    public function fetchAndStore(Carbon $date): bool
    {
        ...
        if ($sourceId === null) {
            Log::channel('daily')->warning(
                "SpacexNycJobExecutionReportService: report_source dengan service_name '" . self::SERVICE_NAME . "' tidak ditemukan. "
        ...
        try {
            ...
            $response = $this->grafana->msearch(
                ...
                [
                    ...
                    'query' => [
                        'bool' => [
                            'must'   => [
                                ['query_string' => ['query' => 'log_category:"BATCH_JOB" AND server:"newyork_dc"']],
                            ],
                            'filter' => [
                                [
                                    'range' => [
            ...
            if ($rows->isEmpty()) {
                Log::warning("SpacexNycJobExecutionReport: tidak ada data untuk {$dateStr}");

                return false;
            }

            foreach ($rows as $doc) {
                SpacexNycJobExecutionReport::updateOrCreate(
                    [
            ...
            Log::info("SpacexNycJobExecutionReport: berhasil simpan {$rows->count()} baris untuk {$dateStr}");

            return true;
        } catch (\Throwable $e) {
            Log::error("SpacexNycJobExecutionReport: gagal - {$e->getMessage()}");
